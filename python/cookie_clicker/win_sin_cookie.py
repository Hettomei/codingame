...
def run():
    ...
    while True:
        i += 0.2
        movex = xx+round(math.cos(i)*size)
        movey = yy+round(math.sin(i)*size)
        ctypes.windll.user32.SetCursorPos(movex, movey)
        _sendMouseEvent(MOUSEEVENTF_LEFTCLICK, movex, movey, count=100)
        time.sleep(0.2)
        killx, killy = _position()
        if killy != movey or killx != movex or killx <= 15:
            print("STOP")
            sys.exit(0)
